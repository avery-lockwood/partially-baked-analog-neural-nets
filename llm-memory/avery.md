...
- **Parallel technical tracks:** 3D printing & AFRB (RepRapFirmware/Duet 3,
  load-cell-instrumented, CF nylon on Creality K2); museum exhibit hardware
  (servo-driven interactive installations with MG996R reliability tracking);
  high-speed mechanical design (~30k RPM spinning wheel, helical gears, 
  material-flow camera systems); electronics troubleshooting (Lepel LSS 
  induction heater, laser diode modules, USB endoscope drivers); local AI 
  ("averybot" — LLM fine-tuning on GTX 1070 with transformers+PEFT+bitsandbytes,
  LoRA rank 16, dolphin-2.9.1-mistral-7b, GGUF serving via Ollama); Linux
  systems (Fedora workstation, NVIDIA drivers, LVM partitions, tmux/Zellij,
